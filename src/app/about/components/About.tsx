...
import AnimeFunFactCard from './AnimeFunFactCard';
...
const SectionWithBrightness = ({ children, className = '', threshold = 0.6 }: { children: React.ReactNode, className?: string, threshold?: number }) => {
    const { ref, inView } = useInView({ threshold });
    return (
        <section
            ref={ref}
            className={`rounded-2xl border border-gray-700 p-8 bg-black/60 transition-all duration-500 ease-in-out ${inView ? 'brightness-125' : 'brightness-50'} ${className}`}
        >
            {children}
        </section>
    );
    ...
    return (
        <div className="w-full flex flex-col items-center mt-4 pb-8 px-4">
            ...
            <div className="max-w-3xl w-full mx-auto">
                <h1 className="text-4xl md:text-5xl font-bold text-center mt-12 mb-2">It's so nice to meet you, You can call me Pramodh <span className="inline-block">👋</span></h1>
                <p className="text-center text-lg mb-8 text-gray-400">Welcome to my digital space!</p>
            </div>
            {/* Single Carousel */}
                ...
                >
                    ...
                    >
                        {imageList.map((src, idx) => (
                            ...
                            >
                                <Image src={src} alt={`about-img-top-${idx}`} width={180} height={220} className="object-contain w-full h-full" />
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <div className="max-w-3xl w-full mx-auto flex flex-col gap-8">
                <SectionWithBrightness>
                    ...
                    <p className="text-base leading-7 text-gray-200 dark:text-gray-300">
                        It all started in a tiny gaming store near my house—the kind with a row of dusty PlayStations and a timer ticking down for every rupee spent. I used to sneak out just for a few minutes of gameplay, but those moments sparked a lifelong obsession with technology.<br/><br/>
                        That curiosity deepened when my dad brought home an old computer. I spent hours playing GTA Vice City, but I wasn't just playing—I was exploring. Friends introduced me to game hacks and tricks, and soon I was flashing custom ROMs, booting phones, and running scripts to crack WiFi passwords. At the time, it all felt like magic. And I wanted to be the magician.<br/><br/>
                        Driven by a love for physics, math, and how machines work, I pursued a Bachelor's in Electrical Engineering. I was fascinated by circuits, embedded systems, and the raw mechanics behind how things run. But somewhere along the way, I realized I didn't just want to understand machines—I wanted to build smarter systems with them. That's when my passion for programming kicked in.<br/><br/>
...
🏍️ <span className="font-bold italic">Bikes & Travel</span>: Nothing beats a good ride on two wheels. I'm always up for spontaneous road trips, especially when there's good food or a scenic view at the end.<br/>
<span className="text-gray-400 block mt-2">“Jobs fill your pockets, but adventures fill your soul.”</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col md:flex-row-reverse items-center gap-8 w-full">
                            ...
                            <div className="flex-1 flex flex-col justify-center md:pr-8 mt-6 md:mt-0">
                                <div className="text-sm md:text-base text-gray-100">
⚽ <span className="font-bold italic">Football & FIFA</span>: Football stole my heart back in college—and FIFA keeps the obsession alive. I'm that friend who never skips match day. One day, I'd love to help my favorite club grow with code.<br/>
<span className="text-gray-400 block mt-2">“It's not just a game. It's a way of life.”</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col md:flex-row items-center gap-8 w-full">
                            ...
                            <div className="flex-1 flex flex-col justify-center md:pl-8 mt-6 md:mt-0">
                                <div className="text-sm md:text-base text-gray-100">
📸 <span className="font-bold italic">Amateur Photographer</span>: I shoot for fun—on trips, during rides, or just random street scenes. Still figuring out the perfect shot, but hey, half the fun is getting there.<br/>
<span className="text-gray-400 block mt-2">“You don't take a photograph, you make it.”</span>
                                </div>
                            </div>
                        </div>
                    </div>
